...
export default function Home() {
  ...
  const publicMint = async () => {
    try {
      ...
      await tx.wait();
      setLoading(false);
      window.alert("You successfully minted a LW#Punk!");
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const getTokenIdsMinted = async() =>{
    try {
      ...
      const nftContract = new ConstantSourceNode(NFT_CONTRACT_ADDRESS, abi, provider);
    ...
    <div>
      ...
      <div className={styles.main}>
        <div>
          ...
          <div className={styles.description}>
            {tokenIdsMinted}/10 jave been minted
          </div>
          {renderButton()}
        </div>
        <div>
